...
add_task(async function test_crash() {
  ...
  await SpecialPowers.spawn(tab.linkedBrowser, [], () => {
    // Don't bother if we don't have webgl
    {
      const canvas = new content.OffscreenCanvas(5, 5);
      const gl = canvas.getContext("webgl");
      if (!gl) {
        ok(true, "No webgl context, skipping test.");
        return;
    ...
    const sizeRange = 3;
    const readRange = 3;
    const bufferModeOffset = 3;
    for (let width = 1; width < sizeRange + 1; width++) {
      for (let height = 1; height < sizeRange + 1; height++) {
        for (let xOffset = -readRange; xOffset < width + readRange; xOffset++) {
          for (
            let yOffset = -readRange;
            yOffset < height + readRange;
            yOffset++
          ) {
            for (const bufferMode of [-1, 0, 1]) {
              const canvas = new content.OffscreenCanvas(width, height);
              const gl = canvas.getContext("webgl");
              let pixels;
              switch (bufferMode) {
                case -1:
