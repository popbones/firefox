...
add_task(async function test_splitViewCreateAndAddTabs() {
  ...
  Assert.ok(
    splitview.tabs[0].hasAttribute("split-view-first"),
    "The first split view tab has the expected attribute"
  );
  Assert.ok(
    splitview.tabs[1].hasAttribute("split-view-second"),
    "The second split view tab has the expected attribute"
  );
  Assert.ok(
