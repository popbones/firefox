...
class SymmetricKeyTest : public testing::Test {
 protected:
  ...
  void SetUp() override {
    if (!NSS_IsInitialized()) ASSERT_EQ(NSS_NoDB_Init(nullptr), SECSuccess);
...
class BenchECDHRunner : public BenchRunner {
 public:
  void Warmup() override { mozilla::UniquePK11SymKey(DeriveKey()); }
...
std::string MakeTestName(const testing::TestParamInfo<size_t>& info) {
  if (info.param == 0) return "warmup";

  if (info.param < KILOBYTE) return std::to_string(info.param);

  if (info.param < MEGABYTE) return std::to_string(info.param / KILOBYTE) + "k";
