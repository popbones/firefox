...
class TestEtcHostsParsing : public ::testing::Test {
 protected:
  ...
  void CreateLockedHostsFile() {
    ...
    rv = mLockedHostsFile->GetNativeTarget(nativePath);
    ASSERT_TRUE(NS_SUCCEEDED(rv));
  ...
  void CreateNoPermissionFile() {
    ...
    nsAutoCString nativePath;
    rv = mNoPermHostsFile->GetNativeTarget(nativePath);
    ASSERT_TRUE(NS_SUCCEEDED(rv));
...
TEST_F(TestEtcHostsParsing, ParseNormalFile) {
  ...
  nsAutoCString path;
  nsresult rv = mTestHostsFile->GetNativeTarget(path);
  ASSERT_TRUE(NS_SUCCEEDED(rv));
...
TEST_F(TestEtcHostsParsing, ParseLockedFile) {
  ...
  nsAutoCString path;
  nsresult rv = mLockedHostsFile->GetNativeTarget(path);
  ASSERT_TRUE(NS_SUCCEEDED(rv));
...
TEST_F(TestEtcHostsParsing, ParseNoPermissionFile) {
  ...
  nsAutoCString path;
  nsresult rv = mNoPermHostsFile->GetNativeTarget(path);
  ASSERT_TRUE(NS_SUCCEEDED(rv));
...
TEST_F(TestEtcHostsParsing, EmptyFile) {
  ...
  nsAutoCString path;
  rv = mTestHostsFile->GetNativeTarget(path);
  ASSERT_TRUE(NS_SUCCEEDED(rv));
